...
//模拟IIC口列表初始化
IIC_Simulation_GPIO_TypeDef IIC_GPIO_List[IIC_SIMULATION_MASTER_NUM] =
{
	{
		.scl_port = GPIOE,
		.scl_pin = GPIO_PIN_6,
...
uint8_t IIC_Simulation_Master_Write(pIIC_Master_WRInfo_TypeDef piic)
{
	...
	while(resend_times--)
	{
        ...
        {
            if(IIC_Simulation_Master_SendByte(piic->port, (uint8_t)(piic->device_adr & 0xFE)))  //设备地址 + 写信号，bit0=0为写，bit0=1为读
            {
                IIC_Simulation_Master_Stop(piic->port);
                continue;  //发送出错就停止，跳过后面循环体语句，直接到while循环条件判断，重新启动IIC和发送
            }
...
uint8_t IIC_Simulation_Master_ReadDirect(pIIC_Master_WRInfo_TypeDef piic)
{
	...
	while(resend_times--)
	{
        ...
        if(piic->dev_adr_tenbit_flag)  //从机为10位地址模式判断
        {
            ...
            if(IIC_Simulation_Master_SendByte(piic->port, (uint8_t)((piic->device_adr & 0xFE) + 1)))  //设备低地址 + 读信号，bit0=0为写，bit0=1为读，注意运算符优先级+高于&
            {
                IIC_Simulation_Master_Stop(piic->port);
                continue;  //发送出错就停止，跳过后面循环体语句，直接到while循环条件判断，重新启动IIC和发送
            }
...
uint8_t IIC_Simulation_Master_ReadRegister(pIIC_Master_ReadReg_Info_TypeDef piic)
{
	...
	while(resend_times--)
	{
        ...
        {
            if(IIC_Simulation_Master_SendByte(piic->wr_info.port, (uint8_t)(piic->wr_info.device_adr >> 8)))  //设备高地址
            {
                IIC_Simulation_Master_Stop(piic->wr_info.port);
                continue;  //发送出错就停止，跳过后面循环体语句，直接到while循环条件判断，重新启动IIC和发送
            }
            if(IIC_Simulation_Master_SendByte(piic->wr_info.port, (uint8_t)(piic->wr_info.device_adr & 0xFE)))  //设备低地址 + 写信号，bit0=0为写，bit0=1为读
            {
                IIC_Simulation_Master_Stop(piic->wr_info.port);
                continue;  //发送出错就停止，跳过后面循环体语句，直接到while循环条件判断，重新启动IIC和发送
            }
        ...
        {
            if(IIC_Simulation_Master_SendByte(piic->wr_info.port, (uint8_t)(piic->wr_info.device_adr & 0xFE)))  //设备地址 + 写信号，bit0=0为写，bit0=1为读
            {
                IIC_Simulation_Master_Stop(piic->wr_info.port);
                continue;  //发送出错就停止，跳过后面循环体语句，直接到while循环条件判断，重新启动IIC和发送
            }
        ...
        {
            if(IIC_Simulation_Master_SendByte(piic->wr_info.port, (uint8_t)(piic->register_adr & 0xFF)))  //寄存器地址
            {
                IIC_Simulation_Master_Stop(piic->wr_info.port);
                continue;  //发送出错就停止，跳过后面循环体语句，直接到while循环条件判断，重新启动IIC和发送
            }
        ...
        if(piic->wr_info.dev_adr_tenbit_flag)  //从机为10位地址模式判断
        {
            ...
            if(IIC_Simulation_Master_SendByte(piic->wr_info.port, (uint8_t)((piic->wr_info.device_adr & 0xFE) + 1)))  //设备低地址 + 读信号，bit0=0为写，bit0=1为读
            {
                IIC_Simulation_Master_Stop(piic->wr_info.port);
                continue;  //发送出错就停止，跳过后面循环体语句，直接到while循环条件判断，重新启动IIC和发送
            }
        ...
        {
            if(IIC_Simulation_Master_SendByte(piic->wr_info.port, (uint8_t)((piic->wr_info.device_adr & 0xFE) + 1)))  //设备地址 + 读信号，bit0=0为写，bit0=1为读
            {
                IIC_Simulation_Master_Stop(piic->wr_info.port);
                continue;  //发送出错就停止，跳过后面循环体语句，直接到while循环条件判断，重新启动IIC和发送
            }
