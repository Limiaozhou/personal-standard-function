...
void CLK_SYSCLK_Config(CLK_Source_Select_TypeDef clk_sel, uint8_t hsidiv, uint8_t cpudiv)
{
    switch(clk_sel)
	{
		...
		case CLK_Source_Select_LSI :  //内部低速时钟，128KHz
			CLK_ICKR |= 0x08;  //低速内部RC振荡器使能
			while(!(CLK_ICKR & 0x10));  //等待低速内部振荡器准备就绪
			CLK_SWCR &= ~((1<<2)|(1<<3));  //时钟切换中断禁止并清除中断标志
			CLK_SWR = CLK_SOURCE_LSI;  //LSI为主时钟源
			while(CLK_SWCR & 0x01);  //等待时钟切换忙结束
			CLK_SWCR |= 0x02;  //使能时钟切换
			while(CLK_CMSR != CLK_SOURCE_LSI);  //等待系统时钟切换到0xD2：LSI
